...
function getCostSummarySelect(prefix = ''): string {
  ...
  return `sumIf(${ref('estimatedCost')}, ${ref('estimatedCost')} IS NOT NULL) AS estimatedCost, CASE WHEN countDistinctIf(${ref('costUnit')}, ${ref('costUnit')} IS NOT NULL) = 1 THEN minIf(${ref('costUnit')}, ${ref('costUnit')} IS NOT NULL) ELSE NULL END AS costUnit`;
}
...
function normalizeCostSummaryRow(row: Record<string, unknown>): CostSummary {
  return {
    ...
    costUnit: row.costUnit == null || row.costUnit === '' ? null : String(row.costUnit),
  };
}
